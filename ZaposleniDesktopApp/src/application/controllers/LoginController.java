...
public class LoginController {
	...
	private void login() {
		...
		Invocation.Builder inv = target.path("login").request(MediaType.APPLICATION_JSON);
		...
		if (bool) {
			...
			cancleAction();
			Loader.openWindow(
					"bin" + File.separator + "application" + File.separator + "view" + File.separator + "Chat.fxml",
					username, 913, 660, true, false, new File("images" + File.separator + "Chat.png"));

		} else
			Loader.showMessage("Korisnicko ime i lozinka nisu validni ili je vas nalog brokiran.");
	}
}
